...
### 5.1.2 inspect selected variables
...
#### 5.1.3.1 First, randomising the rows so that the statements from different eras of the economic movements can be well represented
...
#### 5.1.3.2 Preliminary data cleansing: convert the statements' textual contents to lower and remove `the federal open market committee` and `committee` as it is present in all the statements
...
#### 5.1.3.3 Data Preparation: Here, we prepare the data so that it can be reused for all the classifications without the need to repeat the leaning and preparation processes gaain
...
### 5.2.1 From here, were are going to perform classifications based on `Medium.Term.Rate`, `Employment.Growth`, `Economic.Growth`, `Inflation`, and `Policy.Rate` variables


`Medium.Term.Rate`

### 5.2.2 Classification targetting the `Medium.Term.Rate` variable
...
### 5.2.3 Partition the data into training and test sets
...
### 5.2.4 Classification targeting the `Economic.Growth` variable
...
### 5.2.5 Partition the data into training and test sets: note that the ratios here are different from the other models
...
### 5.2.6 Classification targeting the `Inflation` variable
...
### 5.2.7 remove columns that will not contribute meaninfully to the model fitting
...
### 5.2.8 Classification targeting the `Employment.Growth` variable
...
### 5.2.9 Classification targeting the `Policy.Rate` variable
...
results <- tibble(variable = c("Medium.Term.Rate","Employment.Growth","Economic.Growth","Inflation","Policy.Rate"), modelling = c("80 : 20", "80 : 20", "58 : 42", "70 : 30", "80 : 20"), accuracy= c(100, 90, 79.1, 64.52, 95)) 
 ...
 kable(results,
      col.names = linebreak(c("Variable", "Accuracy (%)", "Modelling (Train : Test)"), align = "c")) %>%
